...
func TestLOMSServerCancelOrder(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(lomsService *controllermocks.MocklomsService) {
				lomsService.EXPECT().
					CancelOrder(gomock.Any(), int64(1001)).
					Return(nil)
			},
			wantErr:     false,
			wantNilResp: false,
		...
		},
		{
			...
			setupMocks: func(lomsService *controllermocks.MocklomsService) {
				lomsService.EXPECT().
					CancelOrder(gomock.Any(), int64(1002)).
		...
		},
		{
			...
			setupMocks: func(lomsService *controllermocks.MocklomsService) {
				lomsService.EXPECT().
					CancelOrder(gomock.Any(), int64(404)).
		...
		},
		{
			...
			setupMocks: func(lomsService *controllermocks.MocklomsService) {
				lomsService.EXPECT().
					CancelOrder(gomock.Any(), int64(1003)).
