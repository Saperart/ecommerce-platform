...
func TestLOMSServicePayOrder(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			setupMocks: func(
				...
				orderRepository.EXPECT().
					SetOrderStatus(gomock.Any(), int64(1003), entity.OrderStatusPaid).
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := service.PayOrder(context.Background(), test.orderID)
